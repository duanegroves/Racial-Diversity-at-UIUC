   ...
   with open(csvFilePath, encoding='utf-8-sig') as csv_file:
      ...
      for idx, row in enumerate(csvreader, start=1):
         ...
         if idx == 1:
            ...
            foundExistingMajor = False
            for saved_idx, saved_row in enumerate(data):
               if saved_row[3] == row[3] and saved_row[4] == row[4]:
                  foundExistingMajor = True
                  data[saved_idx] = [int(row[i]) + int(saved_row[i]) if 7 < i and i < 28 else row[i] for i, el in enumerate(row)]
            if not foundExistingMajor:
               data.append(row)
